...
class SnakeNet1D(nn.Module):
    def __init__(self, dim):
        ...
        # Encoder remains the same. Padding 2->3 for stride 2,4,4, -> 1,2,2 
        self.enc1 = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=1024, kernel_size=8, stride=2, padding=3),  
            nn.BatchNorm1d(1024),
            nn.ReLU()
        )
        self.enc2 = nn.Sequential(
            nn.Conv1d(in_channels=1024, out_channels=2048, kernel_size=8, stride=2, padding=3), 
            nn.BatchNorm1d(2048),
            nn.ReLU()
        )
        self.enc3 = nn.Sequential(
            nn.Conv1d(in_channels=2048, out_channels=512, kernel_size=8, stride=1, padding=3),  
            nn.BatchNorm1d(512),
            nn.ReLU()
        )
        
        # Decoder re-designed to solely use transposed convolutions.
        # Adjusted channel dimensions to account for the removal of concatenations.
        # Encoder remains the same. Padding 2->3 for stride 2,4,4, -> 1,2,2 
        self.dec1 = nn.Sequential(
            nn.ConvTranspose1d(in_channels=512, out_channels=1024,
                               kernel_size=8, stride=1, padding=3, output_padding=0),
            nn.BatchNorm1d(1024),
            nn.ReLU()
        )
        # in_channels=3072 for skip, 1024 for regular
        self.dec2 = nn.Sequential(
            nn.ConvTranspose1d(in_channels=3072, out_channels=512,
                               kernel_size=8, stride=2, padding=3, output_padding=0),
            nn.BatchNorm1d(512),
            nn.ReLU()
        )
        # in_channels=1536 for skip, 512 for regular
        self.dec3 = nn.ConvTranspose1d(in_channels=1536, out_channels=1,
                                       kernel_size=8, stride=2, padding=3, output_padding=0)
